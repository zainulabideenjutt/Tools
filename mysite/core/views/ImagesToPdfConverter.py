...
class ImagesToPdf(generics.ListCreateAPIView, generics.RetrieveAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        pdfName = storagePathFromDomain+randomString+'.pdf'
        data = {'pdfName': pdfName}
        allimages[0].save(pdf_path, "PDF", resolution=100,
                          save_all=True, append_images=allimages[1:])
        ...
        if kwargs:
            ...
            if (type(id) == type(1)):
                ...
                if instance:
                    domain = request.get_host()
                    storagePathFromDomain = f"http://{domain}/media/"
                    pdfPathFromDomain = instance.pdfName
                    pdfName = pdfPathFromDomain.removeprefix(
                        storagePathFromDomain)
                    basePath = Path(__file__).resolve().parent.parent.parent
